...
在它之前，我也试过其他方案。Hugo 构建飞快，但 Go 模板语法让人头疼——官方文档自己都承认"新用户最常犯的错误与上下文（the dot）有关"。一个 `{{ . }}` 的作用域问题能让你调试一下午，报错信息是令人困惑的 `ZgotmplZ`。Hexo 生态丰富，中文教程遍地，但第一步就是装 Node.js + npm + Git，华为云社区有篇文章标题直接叫《Hexo 更新到 5.2.0 版本血泪踩坑史》。V2EX 上有人说"升级了一下 Node，出大问题了"——从 v12 升到 v16，Hexo 直接罢工。Jekyll 更不用说了，Ruby 的版本地狱是出了名的，有人专门写了一篇《Ruby Versioning Hell with Jekyll》来吐槽 Bundler 和 Gem 的版本互锁。
...
用 Hugo，你得学 Go 模板语法——有篇文章叫《Hugo Probably Isn't For You》，评价 Go 模板是 "yet another ad hoc programming language"，不是 HTML，不是 JavaScript，是一种你在其他任何地方都用不到的东西。
...
用 Jekyll，你得先搞定 Ruby 环境。一位开发者说："As someone who has had to install the Ruby/Gem/Jekyll stack quite a few times, I know the pain." 最后他用 Docker 来跑 Jekyll，只为了绕过版本冲突。
...
- 用户的电脑上**不需要安装 Git**，下载 Gridea Pro 就能直接部署
...
**Memo 速记。** 不是每个想法都值得写成一篇完整的文章。有些念头，一句话就够了。Gridea Pro 内置了速记功能，打开就写，支持标签分类，还有热力图帮你追踪记录频率。它像是博客的"朋友圈"——只不过这个朋友圈完全属于你。

**评论系统集成。** 独立博客最大的遗憾之一是缺少互动。Gridea Pro 直接集成了 Gitalk、Giscus、Valine、Waline、Twikoo、Disqus、Cusdis 七种评论方案，在设置页面勾选即用，不需要改一行代码。
...
你在 Gridea Pro 里写的所有东西——文章、速记、配置、图片——全部以 Markdown 和 JSON 文件的形式保存在你本地的文件夹里。没有云端数据库，没有账号系统，没有任何形式的数据锁定。

你可以用 VS Code 打开同一个目录编辑。可以用 iCloud、OneDrive 同步到多台设备。可以随时整个文件夹打包备份。即使有一天 Gridea Pro 不再维护（希望不会），你的 Markdown 文件可以被 Hugo、Hexo、Astro 或任何工具读取。
